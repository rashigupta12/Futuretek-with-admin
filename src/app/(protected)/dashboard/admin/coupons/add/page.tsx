      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-600">Create general purpose or personal coupons</p>
          </div>
        </div>
      </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-3">
                ...
                <span className="text-gray-500 text-sm font-normal ml-1">
                  - Select specific courses this coupon applies to. Leave empty for all courses.
                </span>
              </label>
              ...
              {selectedCourses.length > 0 && (
                <p className="text-sm text-green-600 mt-2">
                  Coupon will be restricted to {selectedCourses.length} selected course(s)
                </p>
              )}
            </div>
          )}
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <code className="text-lg font-mono font-bold text-blue-600">
              {formData.couponTypeId && formData.discountValue
                ? `ADM${couponTypes.find(t => t.id === formData.couponTypeId)?.typeCode}${formData.discountValue.replace(".", "")}XXXX`
                : "COUPON_CODE"}
            </code>
          </div>
          <div className="text-sm text-gray-600 space-y-1">
            <p>
              <strong>Type:</strong> {couponTypes.find(t => t.id === formData.couponTypeId)?.typeName || "N/A"}
            </p>
            <p>
            ...
            {formData.couponScope === "GENERAL" && selectedCourses.length > 0 && (
              <p>
